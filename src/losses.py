...
def TotalReconstructionLoss(
    output, captions, features=None, features_recons=None, reg_lambda=0, recon_lambda=0, reconstruction_type="global",
):
...
def ModalityWiseReconstructionLossBuilder(reg_lambda, audio_recon_lambda, visual_recon_lambda, rec_type="none"):
    assert rec_type in ["none", "global", "local",], "Wrong mode specified, must be one of ['none', 'global', 'local']"
    return partial(
...
if __name__ == "__main__":
    ...
    audio_features, audio_features_recons = (
        torch.rand([batch_size, seconds, audio_feature_size]),
        torch.rand([batch_size, seconds, audio_feature_size]),
    )
    visual_features, visual_features_recons = (
        torch.rand([batch_size, seconds, audio_feature_size]),
        torch.rand([batch_size, seconds, audio_feature_size]),
    )
    ...
    print(
        ...
        entropy_loss.shape,
        audio_reconstruction_loss.shape,
        visual_reconstruction_loss.shape,
    )
