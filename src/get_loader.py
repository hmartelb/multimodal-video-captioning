...
def read_MSR_VTT_Metadata(root_dir, split):
    ...
    metadata = pd.DataFrame(data["sentences"])
    metadata["id"] = metadata.video_id.apply(lambda x: int(x.replace("video", "")))
    start_end = {"train": [0, 6512], "val": [6513, 7009], "test": [7010, 9999]}  # (6513)  # (497)  # (2990)
...
class VideoCaptionsDataset(Dataset):
    def __init__(
        self, root_dir, vid_cap_dict, normalize=False, video_only=False,
    ):
        """
...
def VideoDataset_to_VideoCaptionsLoader(
    videodataset, batch_size=32, num_workers=0, normalize=False, video_only=False,
):
    ...
    videoCaptionsDataset = VideoCaptionsDataset(
        videodataset.root_dir, vid_captions_dict, normalize=normalize, video_only=video_only
    )
...
def get_loader(
    ...
    dataset = VideoCaptioningDataset(
        root_dir, dataset=dataset, split=split, vocab_pkl=vocab_pkl, normalize=normalize, video_only=video_only
    )
...
if __name__ == "__main__":
    ...
    val_loader, val_dataset = get_loader(
        root_dir=dataset_folder, dataset=dataset, split="val", batch_size=128, vocab_pkl=vocab_pkl
    )
